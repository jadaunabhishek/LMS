...
class UserBooksModel: ObservableObject{
        ...
        dbInstance.collection("Books").getDocuments{ (snapshot, error) in
            ...
            if(error == nil && snapshot != nil){
                for document in snapshot!.documents{
                    ...
                    tempAllBooks.append(book)
                }
                self.allBooks = tempAllBooks
                self.authors = tempAuthor
            }
    ...
    func requestBook(bookId: String, bookName: String, bookImageURL: String, userId: String, userName: String, bookAvailableCount: Int, bookTakenCount: Int, loanPeriod: Int){
    ...
    func preBook(bookId: String, bookName: String, bookImageURL: String, userId: String, userName: String, bookPreBookedCount: Int, loanPeriod: Int){
        ...
        self.dbInstance.collection("Books").document(bookId).updateData(["bookPreBookedCount": bookPreBookedCount+1, "updatedOn": Date.now.formatted()]){ error in
            ...
            if let error = error{
                ...
                let newLoan = Loan(loanId: addNewLoan.documentID, bookId: bookId, bookName: bookName, bookImageURL: bookImageURL, bookIssuedTo: userId, bookIssuedToName: userName, bookIssuedOn: Date.now.formatted(), bookExpectedReturnOn: (Calendar.current.date(byAdding: .day, value: loanPeriod, to: Date.now)?.formatted())!, bookReturnedOn: "", loanStatus: "PreBooked", loanReminderStatus: "notSet", fineCalculatedDays: 0, loanFine: 0, createdOn: Date.now.formatted(), updatedOn: Date.now.formatted(), timeStamp: retrievedSort)
        ...
        self.dbInstance.collection("Books").document(bookId).getDocument { (document, error) in
            if error == nil{
                if (document != nil && document!.exists){
                    ...
                    var newRating = Float((Int(tempCurrentBook[0].bookRating)+rating)/(tempCurrentBook[0].bookReviews.count+1))
                    
                    self.dbInstance.collection("Books").document(bookId).updateData(["bookRating":newRating, "bookReviews":tempCurrentBook[0].bookReviews.append(review), "updatedOn": Date.now.formatted()])
                    self.responseStatus = 200
                    self.responseMessage = "Book fetched successfuly"
                }
                else{
